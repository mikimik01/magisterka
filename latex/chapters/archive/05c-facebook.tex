...
Jedna iteracja (≈ 12{,}3 min) składała się z: (1) zimnego startu i krótkiej nawigacji (feed, profil, wyszukiwanie), (2) krótkiej interakcji multimedialnej, (3) przejścia do tła i obserwacji w tle. Szczegóły i uzasadnienia czasów — patrz Rozdz.\ \ref{chap:metodyka}.

\section{Zebrane dane (ta sesja)}
Okno eksperymentu: \textbf{2025-08-19 19{:}24{:}12–19{:}36{:}29} (≈ \SI{12.28}{\minute}). Zarejestrowano \textbf{305} przepływów (\emph{flows}) o łącznym wolumenie \textbf{≈\,29.75 MiB}; \textbf{RX ≈\,28.30 MiB}, \textbf{TX ≈\,1.45 MiB}.
...
\begin{itemize}
  \item \textbf{Dominacja QUIC/HTTP/3:} niemal cały wolumen (≈98{,}5\%) to QUIC na 443/UDP; obecne są też śladowe HTTPS/TCP na 443 (np.\ fallback, zasoby pomocnicze).
  ...
  \item \textbf{Wzorzec ruchu:} wyraźny \emph{burst} około 19{:}26 (większość nowych i kończących się flow), po czym aktywność w tle spada do sporadycznych wywołań.
\end{itemize}
...
W badanej sesji Facebook wykazuje: (i) dominację QUIC/HTTP/3 w wolumenie (≈98{,}5\%), (ii) charakterystyczne kanały usługowe (\texttt{graph}/\texttt{gateway}/\texttt{edge-mqtt}), (iii) krótkotrwałe piki transferów związane z interakcjami użytkownika oraz ograniczoną aktywność w tle. Z perspektywy prywatności kluczowe są uprawnienia do lokalizacji, multimediów i pracy w tle oraz obecność komponentów analityczno-reklamowych — ich interpretację należy zawsze osadzać w kontekście wersji aplikacji i potwierdzać obserwacjami dynamicznymi.
