...
(\textbf{\emph{RX} $\approx$ 0.15 MiB}, \textbf{\emph{TX} $\approx$ 0.06 MiB}).
...
Dominuje \textbf{\emph{HTTPS}/\emph{TLS}}; pojedynczy ślad \textbf{\emph{HTTP}/80} dotyczy hosta Google (prawdopodobnie kontrola łączności/zasób systemowy) i nie wskazuje na przesyłanie treści aplikacyjnych w badanym oknie czasowym.
...
\paragraph{Obserwacja:}
Ruch koncentruje się na 443/\emph{TCP} do usług państwowych; widoczny jest także kanał \emph{TLS} na nietypowym porcie \textbf{3000} (telemetria/błędy).
...
\begin{itemize}
  \item \textbf{Szyfrowanie transportowe:} właściwa komunikacja aplikacji po \emph{TLS}/443; brak treści w badanym oknie czasowym.
  \item \textbf{Brak \emph{QUIC}/\emph{HTTP/3}} w zarejestrowanym wycinku — kanał klasyczny \emph{TLS}/\emph{TCP}.
...
Ze względu na \emph{TLS} treści zapytań/odpowiedzi nie są dostępne; nie widać przesyłania danych wrażliwych w badanym oknie czasowym.

\paragraph{Praktyki bezpieczeństwa.}
W analizowanym wycinku widać konsekwentne użycie \emph{HTTPS}/\emph{TLS} do usług \texttt{*.gov.pl}, brak nietypowych punktów końcowych poza kanałem telemetrycznym (\emph{TLS}/3000) i brak szerokich odwołań do domen trzecich. To zachowanie spójne z wysokim standardem bezpieczeństwa na warstwie transportowej. Pełna ocena (np. pinning certyfikatów, mechanizmy \emph{anti-tamper}/\emph{anti-root}, magazyn kluczy, ochrona ekranu) wymaga analizy \emph{APK} oraz testów dynamicznych.

\subsection{Podsumowanie}
W badanej sesji mObywatel wykazuje:
\begin{enumerate}
\item dominację \emph{TLS}/\emph{HTTPS}; wolumen ($\approx 0.21$ MiB), \textbf{13} przepływów w $\sim$\SI{2.9}{\minute},
\item koncentrację ruchu na \texttt{api.mobywatel.gov.pl} oraz wewnętrzną telemetrię \texttt{sentry.puch.coi.gov.pl},
\item pojedynczy flow \emph{HTTP}/80 do zasobów Google (nie dotyczy danych wrażliwych aplikacji).
\end{enumerate}
