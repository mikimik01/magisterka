...
(\textbf{RX $\approx$ 0.06 MiB}, \textbf{TX $\approx$ 0.03 MiB}).

\paragraph{Przebieg badania:} 
Scenariusz obejmował około \textbf{15 minut interakcji} z aplikacją, w tym: 
\emph{start aplikacji, zmianę hasła, logowanie, dodanie rejestracji, nawigację po menu, wylogowanie, próbę błędnego logowania, ponowne logowanie, krótką pracę w trybie offline i online, ponowne wylogowanie oraz około 10 minut działania aplikacji w tle}.
...
Ruch aplikacji jest w całości oparty o \textbf{HTTPS} (brak QUIC oraz HTTP w klarze).
...
\paragraph{Obserwacja:}
Cele to wyłącznie port \textbf{443/TCP} (HTTPS) oraz standardowe \textbf{DNS/53}. Brak nietypowych portów.
...
\paragraph{Obserwacja:}
Widoczne są domeny \texttt{yourb.app} (API/aktualizacje), \texttt{sentry.io} (raportowanie błędów) oraz \texttt{stripe.com} (płatności).

\subsection{Obserwacje jakościowe}
\begin{itemize}
  \item \textbf{Brak QUIC/HTTP/3.} Cała komunikacja po 443/TCP (TLS).
  \item \textbf{Brak HTTP.} W badanej sesji nie obserwowano niezaszyfrowanych żądań HTTP.
...
Ze względu na TLS treści żądań/odpowiedzi nie są dostępne; brak oznak jawnego przesyłania danych.

\paragraph{Hipotezy dot. bezpieczeństwa.}
...
\begin{enumerate}
\item ruch wyłącznie po HTTPS (bez QUIC), niewielki wolumen ($\approx 0.08$ MiB) i zapytania DNS,
\item integrację z \texttt{sentry.io} (telemetria błędów) i \texttt{stripe.com} (płatności),
\item brak śladów HTTP w klarze; dalsze badania wymagane, by ocenić bezpieczeństwo lokalnej bazy danych.
\end{enumerate}
