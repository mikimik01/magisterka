\section{Mobile USOS PP (wersja 1.14.1)}
\label{sec:usos}
...
(\textbf{\emph{RX} $\approx$ 2.47 MiB}, \textbf{\emph{TX} $\approx$ 0.40 MiB}).
...
Ruch USOS w tej sesji jest zdominowany przez \textbf{\emph{HTTPS}/\emph{TLS}} (brak \emph{QUIC} i brak \emph{HTTP} w badanym oknie czasowym).
...
\paragraph{Obserwacja:}
Największy wolumen kierowany jest do hostów na porcie \textbf{443/\emph{TCP}} (\emph{HTTPS}); widoczny jest też kanał \textbf{\emph{TLS}/3000} o znikomym wolumenie (telemetria/błędy).
...
\begin{itemize}
  \item \textbf{Brak \emph{QUIC}/\emph{HTTP/3}.} Komunikacja aplikacyjna po \emph{TLS}/443; treść niedostępna.
  \item \textbf{Brak \emph{HTTP}.} Nie odnotowano żądań nieszyfrowanych.
...
Warstwa transportowa jest poprawnie szyfrowana (\emph{TLS}); brak widoczności treści żądań/odpowiedzi oraz brak jawnego \emph{HTTP}.

\paragraph{Praktyki bezpieczeństwa.}
W analizowanej sesji widać konsekwentne użycie \textbf{\emph{HTTPS}/\emph{TLS}} do hostów uczelnianych oraz ograniczoną telemetrię (Crashlytics/Firebase). Dalsza ocena (pinning certyfikatów, mechanizmy \emph{anti-root}/\emph{anti-tamper}, magazyn kluczy) wymaga analizy \emph{APK} i testów dynamicznych.

\subsection{Podsumowanie}
W badanej sesji USOS wykazuje:
\begin{enumerate}
\item dominację \emph{TLS}/\emph{HTTPS}; wolumen ($\approx 2.88$ MiB), \textbf{200} przepływów w $\sim$\SI{4.3}{\minute},
