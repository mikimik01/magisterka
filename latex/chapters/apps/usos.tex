...
Ruch USOS w tej sesji jest zdominowany przez \textbf{HTTPS/TLS} (brak QUIC i brak HTTP w klarze).
...
\emph{Spostrzeżenia:} 
\begin{itemize}
  \item Exodus wykrywa sygnatury statyczne trackerów — nie jest to dowód ich aktywności w czasie działania.
  \item Jedyny tracker ma charakter czysto diagnostyczny; zestaw uprawnień obejmuje zarówno funkcje systemowe (NFC, notyfikacje), jak i elementy fakultatywne (kalendarz).
\end{itemize}
...
\item brak śladów HTTP w klarze oraz brak QUIC/HTTP/3 w zarejestrowanym oknie.
\end{enumerate}
