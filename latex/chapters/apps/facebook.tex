\section{Facebook (wersja 526.1.0.66.75)}
\label{sec:facebook}

\subsection*{Okno i wolumen}
Okno eksperymentu: \textbf{2025-08-19 19{:}24{:}12–19{:}36{:}29} ($\approx$ \SI{12.28}{\minute}). Zarejestrowano \textbf{305} przepływów, wolumen $\approx$ \textbf{29.75 MiB} (\textbf{\emph{RX} $\approx$ 28.30 MiB}, \textbf{\emph{TX} $\approx$ 1.45 MiB}).

\paragraph{Przebieg badania:} 
Scenariusz obejmował około \textbf{15 minut interakcji} z aplikacją, w tym: 
\emph{nasłuch tła (2 min), logowanie, wylogowanie, oglądanie filmów i zdjęć, "lajkowanie", nawigację po głównych ekranach (offline i online), zamknięcie aplikacji oraz nasłuch tła}.

\subsection{Wolumen i liczność wg protokołu}
\begin{table}[H]
  \centering
  \caption{Podsumowanie wg protokołu (Facebook, ta sesja)}
  \label{tab:fb-proto}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Protokół} & \textbf{Flows} & \textbf{MiB (total)} & \textbf{B wysłane} & \textbf{B odebrane} \\
    \midrule
    QUIC  & 132 & 29.29 & 1{,}189{,}171 & 29{,}522{,}102 \\
    TLS   & 93  & 0.01 & 5{,}620   & 3{,}768 \\
    DNS   & 46  & 0.00 & 1{,}137   & 1{,}778 \\
    HTTPS & 19  & 0.43 & 312{,}835 & 142{,}702 \\
    TCP   & 10  & 0.01 & 9{,}111   & 2{,}899 \\
    HTTP  & 5   & 0.00 & 300       & 200 \\
    \midrule
    \textbf{Razem} & \textbf{305} & \textbf{29.75} & \textbf{1{,}518{,}174} & \textbf{29{,}673{,}449} \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Obserwacja:} 
Ruch Facebooka jest silnie zdominowany przez \emph{QUIC/HTTP/3} (jak u Instagrama). 

\subsection{Największe cele (IP/port) wg wolumenu}
\begin{table}[H]
  \centering
  \caption{Największe kierunki ruchu (Facebook, ta sesja)}
  \label{tab:fb-topdst}
  \begin{tabular}{lllrr}
    \toprule
    \textbf{IP docelowe} & \textbf{Port} & \textbf{Proto} & \textbf{Flows} & \textbf{MiB} \\
    \midrule
    185.60.217.28  & 443 & QUIC  & 50 & 25.46 \\
    157.240.210.14 & 443 & QUIC  & 1  & 2.01 \\
    185.60.217.20  & 443 & QUIC  & 79 & 1.79 \\
    185.60.217.3   & 443 & HTTPS & 2  & 0.20 \\
    157.240.210.16 & 443 & HTTPS & 8  & 0.15 \\
    157.240.210.3  & 443 & HTTPS & 3  & 0.03 \\
    185.60.217.13  & 443 & HTTPS & 2  & 0.02 \\
    185.60.217.28  & 443 & HTTPS & 1  & 0.02 \\
    157.240.0.13   & 443 & QUIC  & 1  & 0.01 \\
    31.13.91.40    & 443 & QUIC  & 1  & 0.01 \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Obserwacja:}
Największy wolumen koncentruje się na adresie \texttt{185.60.217.63} (serwer Meta, Dublin - jak u Instagrama).

\subsection{Najczęstsze hosty (liczba przepływów)}
\begin{table}[H]
  \centering
  \caption{Top hosty (Facebook, liczba przepływów)}
  \label{tab:fb-hosts}
  \begin{tabular}{lr}
    \toprule
    \textbf{Host} & \textbf{Flows} \\
    \midrule
    \texttt{graph.facebook.com} & 85 \\
    \texttt{lookaside.facebook.com} & 54 \\
    \texttt{chat-e2ee-mini.facebook.com} & 25 \\
    \texttt{gateway.instagram.com} & 24 \\
    \texttt{gateway.facebook.com} & 19 \\
    \texttt{edge-mqtt.facebook.com} & 14 \\
    \texttt{graph-fallback.facebook.com} & 14 \\
    \texttt{gateway-fallback.facebook.com} & 9 \\
    \texttt{edge-mqtt-fallback.facebook.com} & 2 \\
    \texttt{*.fbcdn.net} (różne) & 5 \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Obserwacja:} 
Analiza hostów pokazuje silną koncentrację ruchu na \texttt{graph.facebook.com} oraz powiązanych usługach (\texttt{lookaside}, \texttt{chat-e2ee-mini}, \texttt{gateway}). Wskazuje to na dominującą rolę centralnego \emph{API} (\texttt{graph}) i serwisów pomocniczych (logowanie, czat, integracja z Instagramem). Dodatkowa obecność hostów typu \texttt{*-fallback} oraz \texttt{*.fbcdn.net} sugeruje stosowanie mechanizmów redundancji i serwerów \emph{CDN} do dystrybucji treści multimedialnych.

\subsection{Wyniki z Exodus Privacy (aplikacja)}
Raport dla \texttt{com.facebook.katana} (wersja 525.0.0.53.51, Google Play) wskazuje obecność \textbf{8 trackerów} oraz \textbf{90 uprawnień}.

\textbf{Trackery.}
\begin{itemize}
  \item Facebook Ads (advertisement),
  \item Facebook Audience (analytics),
  \item Facebook Flipper (analytics),
  \item Facebook Notifications,
  \item Facebook Places,
  \item Facebook Share,
  \item Google Analytics (analytics),
  \item Mapbox.
\end{itemize}

\textbf{Uprawnienia — przykłady kluczowe:}
\begin{itemize}
  \item \textbf{Lokalizacja:} \texttt{ACCESS\_COARSE\_LOCATION}, \texttt{ACCESS\_FINE\_LOCATION}, \texttt{ACCESS\_MEDIA\_LOCATION}.
  \item \textbf{Multimedia:} \texttt{CAMERA}, \texttt{RECORD\_AUDIO}, \texttt{READ/WRITE\_EXTERNAL\_STORAGE}, \texttt{READ\_MEDIA\_IMAGES/VIDEO}.
  \item \textbf{Tożsamość/konta/telefonia:} \texttt{GET\_ACCOUNTS}, \texttt{READ\_CONTACTS}, \texttt{READ\_PHONE\_STATE}, \texttt{MANAGE\_ACCOUNTS}.
  \item \textbf{Sieć i system:} \texttt{INTERNET}, \texttt{ACCESS\_NETWORK\_STATE}, \texttt{ACCESS\_WIFI\_STATE}, \texttt{NFC}, \texttt{WAKE\_LOCK}, \texttt{SYSTEM\_ALERT\_WINDOW}.
  \item \textbf{Biometria i powiadomienia:} \texttt{USE\_BIOMETRIC}, \texttt{USE\_FINGERPRINT}, \texttt{POST\_NOTIFICATIONS}.
\end{itemize}

\emph{Spostrzeżenia:} 
\begin{itemize}
  \item Zidentyfikowano szerokie spektrum trackerów, głównie od samego Facebooka, a także Google i Mapbox.
  \item Zakres uprawnień jest bardzo szeroki (90), obejmując zarówno krytyczne elementy systemowe (kamera, mikrofon, kontakty, lokalizacja), jak i dodatkowe funkcje (NFC, kalendarz).
  \item Liczba i charakter uprawnień odpowiada aplikacji działającej jako główna platforma społecznościowa, ale równocześnie rodzi poważne obawy o prywatność użytkowników.
\end{itemize}

\textbf{Uprawnienia — kategorie kluczowe.} Dla Facebooka istotne z perspektywy prywatności są zwłaszcza:
\begin{itemize}
  \item \textbf{Lokalizacja:} \texttt{ACCESS\_FINE\_LOCATION}, \texttt{ACCESS\_COARSE\_LOCATION} (precyzyjna/przybliżona lokacja).
  \item \textbf{Multimedia:} \texttt{CAMERA}, \texttt{RECORD\_AUDIO}, dostęp do pamięci (\texttt{READ/WRITE\_EXTERNAL\_STORAGE} — jeśli występują w używanej wersji).
  \item \textbf{Tożsamość/konta/książka adresowa:} \texttt{GET\_ACCOUNTS}, \texttt{READ\_CONTACTS}.
  \item \textbf{Sieć/telefonia i praca w tle:} \texttt{ACCESS\_NETWORK\_STATE}, \texttt{WAKE\_LOCK}, \texttt{RECEIVE\_BOOT\_COMPLETED}.
\end{itemize}
\emph{Uwaga:} dokładny zestaw i liczba uprawnień zależą od wersji aplikacji i kanału dystrybucji; w raporcie Exodusa dla \texttt{com.facebook.katana} należy je cytować dla konkretnej wersji aplikacji poddanej badaniu.

\subsection{Subiektywne spostrzeżenia}
\paragraph{Obserwowalność treści.}
Transport \emph{QUIC/UDP} ogranicza klasyczne \emph{MITM/TLS-over-TCP}; wgląd w semantykę HTTP jest bardzo ograniczony.

\paragraph{Zmienność wersji.}
Facebook to aplikacja o wysokiej dynamice wydawniczej; zestaw hostów, udział QUIC/H2 oraz uprawnienia mogą różnić się między wersjami i regionami. Liczby w tym rozdziale odnoszą się do konkretnej sesji i wersji aplikacji użytej w eksperymencie.

\subsection{Podsumowanie}
W badanej sesji Facebook wykazuje: 
\begin{enumerate}
\item dominację QUIC/HTTP/3 w wolumenie ($\approx$98{,}5\%), 
\item charakterystyczne kanały usługowe (\texttt{graph}/\texttt{gateway}/\texttt{edge-mqtt}), 
\item krótkotrwałe piki transferów związane z interakcjami użytkownika oraz ograniczoną aktywność w tle. Z perspektywy prywatności kluczowe są uprawnienia do lokalizacji, multimediów i pracy w tle oraz obecność komponentów analityczno-reklamowych.
\end{enumerate}