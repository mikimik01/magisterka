...
(\textbf{\emph{RX} $\approx$ 27.20 MiB}, \textbf{\emph{TX} $\approx$ 0.85 MiB}).

\paragraph{Przebieg badania:} 
Scenariusz obejmował około \textbf{15 minut interakcji} z aplikacją, w tym: 
\emph{nasłuch tła (2 min), logowanie, wylogowanie, oglądanie filmów i zdjęć, "serduszkowanie", nawigację po głównych ekranach (offline i online), zamknięcie aplikacji oraz nasłuch tła}.
...
Ruch Instagrama jest silnie zdominowany przez \emph{QUIC/HTTP/3}. 
...
\paragraph{Obserwacja:}
Największy wolumen koncentruje się na adresie \texttt{185.60.217.63} (serwer Meta, Dublin). Występują też długotrwałe połączenia \emph{TCP}/5222 (port \emph{MQTT}) do \texttt{185.60.217.175}, związane najprawdopodobniej z powiadomieniami/czatem.
...
\begin{itemize}
  \item \textbf{Dominacja \emph{HTTP/3}/\emph{QUIC}.} Zdecydowana większość ruchu (liczebnie i wolumenowo) to \emph{QUIC} na 443/\emph{UDP}, co jest zgodne z trendem migracji warstwy mediów do \emph{HTTP/3} \cite{RFC9114,RFC9000}.
