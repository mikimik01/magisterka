...
Rynek aplikacji mobilnych stał się dominującym kanałem konsumpcji treści i usług cyfrowych. Dane o penetracji dostępu mobilnego (ok.\ 71\% globalnej populacji posiada subskrypcję mobilną do 2023 r.) \cite{CiscoAIRWhitepaper2023} oraz statystyki udziału ruchu \emph{WWW} (ponad 50\% globalnych \emph{page views} pochodzi z urządzeń mobilnych) \cite{StatCounterPlatformShare2025} pokazują, że to właśnie środowisko mobilne jest dziś kluczowym miejscem, w którym kształtują się wzorce komunikacji sieciowej i materializują ryzyka dla prywatności.

W ekosystemie Androida współistnieje wiele bibliotek i zestawów \emph{SDK} firm trzecich — od analityki i reklam, przez personalizację, po rozwiązania antyfraudowe. Każda z tych warstw wnosi własne punkty końcowe, protokoły i mechanizmy telemetrii, często działające poza świadomością użytkownika, a niekiedy również poza pełną kontrolą zespołów deweloperskich aplikacji. W efekcie rośnie złożoność oraz nieprzejrzystość przepływów danych.
...
\begin{enumerate}[label=\alph*]
  ...
  \item budowa powtarzalnych narzędzi i procedur, które nadążą za ewolucją protokołów (\emph{HTTP/2, HTTP/3/QUIC}) oraz mechanizmami obronnymi (np.\ \emph{certificate pinning}).
\end{enumerate}

Wobec powyższego potrzebne jest uporządkowane, możliwie obiektywne podejście do \emph{systematycznej} obserwacji i oceny ruchu sieciowego aplikacji: środowisko pomiarowe, które pozwala porównywać różne aplikacje i kategorie, zdefiniowany zestaw metryk (zarówno ilościowych, jak i jakościowych) oraz protokoły postępowania uwzględniające ograniczenia współczesnych technologii transportowych i zabezpieczeń. Tak rozumiana rama badawcza stanowi bezpośrednią motywację dla niniejszej pracy.


Drugi wektor motywacyjny stanowi szybka adopcja nowych protokołów transportowych i warstw aplikacyjnych. \emph{QUIC} (standaryzowany jako \emph{HTTP/3} \cite{RFC9114}) przenosi część logiki transportu do przestrzeni użytkownika (\emph{UDP} + \emph{TLS}~1.3 \cite{RFC8446}), co zmienia charakterystykę obserwowalnych cech przepływów: klasyczne estymatory opóźnień i retransmisji z poziomu \emph{TCP} stają się mniej bezpośrednie, a analiza pakietów jest utrudniona przez szyfrowanie większej liczby pól. W konsekwencji wcześniejsze proporcje protokołów publikowane w literaturze mogą być nieaktualne dla obecnych wersji popularnych aplikacji.

Kolejnym aspektem jest prywatność i otoczenie regulacyjne. RODO (\emph{GDPR}), a także \emph{DMA/DSA} w UE akcentują minimalizację i przejrzystość przetwarzania. Architektura „\emph{multi-endpoint}” (\emph{API} własne + analityka + reklamy + antyfraud + \emph{CDN/edge}) prowadzi do gęstych, równoległych zestawów połączeń tuż po interakcji użytkownika. Nawet przy pełnym szyfrowaniu treści korelacja metadanych (częstotliwość, rozmiary odpowiedzi, domeny) może ujawniać wzorce użycia funkcji (\emph{pattern-of-life}). Transparentność takich wzorców jest ograniczona zarówno dla użytkownika, jak i audytorów. Narzędzia systemowe dają uogólniony obraz, a techniki inwazyjne (\emph{hooking}, \emph{dynamic instrumentation}) bywają blokowane przez mechanizmy \emph{anty‑tampering}.

Istnieje także motywacja praktyczna stworzenia lekkiego, powtarzalnego środowiska badawczego balansującego między:
\begin{itemize}
  \item \textbf{zasięgiem obserwacji} tj. kompletność metadanych \emph{HTTP[S]},
...
\begin{description}
  \item[L1] Brak bieżących, empirycznych pomiarów dystrybucji \emph{HTTP/1.1} vs \emph{HTTP/2} vs \emph{QUIC} w popularnych aplikacjach.
  ...
  \item[L4] Fragmentaryczne dane o wpływie \emph{certificate pinning} na pokrycie przechwytu.
\end{description}

Interesariusze:
\begin{itemize}
  \item \textbf{Użytkownik} – zyskuje pośrednio poprzez \emph{privacy-by-design}.
  ...
  \item \textbf{Społeczność naukowa} – powtarzalny \emph{pipeline} do dalszych badań.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Analiza ruchu sieciowego} – identyfikacja używanych protokołów (\emph{HTTP/1.1}, \emph{HTTP/2}, \emph{QUIC}, \emph{TLS}), badanie bezpieczeństwa transmisji danych (szyfrowanie, \emph{SSL pinning}) oraz klasyfikacja przesyłanych informacji.
  ...
  \item \textbf{Analiza zużycia zasobów} – pomiary obciążenia baterii i \emph{CPU} aplikacji z użyciem \textit{Battery Historian} oraz \textit{Android Profiler}.
